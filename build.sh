...
# Zip Name
ZIPNAME=Poco # Ganti pakai nama kernelnya apa
FINAL_ZIP=${ZIPNAME}-${VERSION}-${DEVICE}-KERNEL-${TM}.zip

# Specify compiler [ proton, neutron, aosp, zyc, gas]
COMPILER=gas
	...
	case $COMPILER in
		...
		zyc)
	     ...
	     gas)
		    ...
		    echo "|| Build Tools ||"
		    mkdir tools
		    git clone https://android.googlesource.com/platform/prebuilts/build-tools ${KERNEL_DIR}/tools/build-tools
		    PATH="${KERNEL_DIR}/clang/bin:$PATH"
		    PATH="${KERNEL_DIR}/tools/build-tools/path/linux-x86:${KERNEL_DIR}/gas/linux-x86:$PATH"
...
function exports() {
        ...
        if [ -d ${KERNEL_DIR}/clang ];
           ...
           then
            ...
            then
               export CROSS_COMPILE=${KERNEL_DIR}/clang/bin/aarch64-linux-gnu-
